        ...
        descriptor.value = async function (...args: any[]) {
            ...
            try {
                ...
                const shouldFailTransaction =
                    options?.shouldFail || TransactionMockState.shouldFail;
                ...
                const index =
                    TransactionMockState.activeTransactions.indexOf(
                        transactionId,
                    );
                if (index > -1) {
                ...
                descriptor.value = async function (...args: any[]) {
                    ...
                    try {
                        ...
                        if (TransactionMockState.shouldFail) {
                            TransactionMockState.failedTransactions.push(
                                transactionId,
                            );
                            throw new Error(
                                `Transaction ${transactionId} rolled back (simulated)`,
                        ...
                        // Transaction succeeded
                        TransactionMockState.completedTransactions.push(
                            transactionId,
                        );
                        return result;
                    } catch (error) {
                        ...
                        if (
                            !TransactionMockState.failedTransactions.includes(
                                transactionId,
                            )
                        ) {
                            TransactionMockState.failedTransactions.push(
                        ...
                        if (index > -1) {
                            TransactionMockState.activeTransactions.splice(
                                index,
                                1,
                            );
                        }
