...
import { AuthApplicationService, IAuthApplicationService, AUTH_APPLICATION_SERVICE_TOKEN } from '../../application/application-services/auth.application-service';
import { ACCOUNT_REPO_TOKEN, IAccountRepository } from '../../application/contracts/account-repository.contract';
import { USER_REPO_TOKEN, IUserRepository } from '../../application/contracts/user-repository.contract';
import { ASSET_SERVICE_TOKEN, IAssetService } from '../../application/contracts/asset-service.contract';
import { AccountNotFoundException, InvalidCredentialsException } from '../../application/exceptions/auth.exception';
...
describe('AuthApplicationService', () => {
  ...
  describe('localLogin', () => {
    ...
    it('should throw AccountNotFoundException when user not found', async () => {
      ...
      // Act & Assert
      await expect(service.localLogin(loginPayload)).rejects.toThrow(AccountNotFoundException);
      expect(accountRepo.findOne).toHaveBeenCalledWith({
        email: loginPayload.email,
        provider: ACCOUNT_PROVIDER.LOCAL,
      });
    ...
    it('should throw InvalidCredentialsException with wrong password', async () => {
      ...
      // Act & Assert
      await expect(service.localLogin(loginPayload)).rejects.toThrow(InvalidCredentialsException);
      expect(hashUtil.validateHashString).toHaveBeenCalledWith(
        loginPayload.password,
        'hashed-password',
      );
  ...
  describe('register', () => {
    it('should register user with all required data', async () => {
      ...
      assetService.createAsset.mockResolvedValue(mockAsset);
      ...
      expect(hashUtil.generateHash).toHaveBeenCalledWith(registerPayload.password);
    ...
    it('should handle asset creation failure', async () => {
      ...
      // Act & Assert
      await expect(service.register(registerPayload)).rejects.toThrow('Asset creation failed');
    ...
    it('should handle user repository insertion failure', async () => {
      ...
      // Act & Assert
      await expect(service.register(registerPayload)).rejects.toThrow('User insertion failed');
      expect(userRepo.insert).toHaveBeenCalled();
      expect(accountRepo.insert).not.toHaveBeenCalled();
    });
    ...
    it('should handle account repository insertion failure', async () => {
      ...
      // Act & Assert
      await expect(service.register(registerPayload)).rejects.toThrow('Account insertion failed');
      expect(userRepo.insert).toHaveBeenCalled();
      expect(accountRepo.insert).toHaveBeenCalled();
    });
  });
