...
describe('AuthApplicationService (Simplified)', () => {
  ...
  describe('localLogin', () => {
    ...
    it('should throw error when account not found', async () => {
      ...
      // Act & Assert
      await expect(service.localLogin(loginPayload)).rejects.toThrow('Account not found');
      expect(mockAccountRepository.findOne).toHaveBeenCalledWith({
        email: loginPayload.email,
        provider: 'LOCAL',
      });
    ...
    it('should throw error with invalid password', async () => {
      ...
      // Act & Assert
      await expect(service.localLogin(loginPayload)).rejects.toThrow('Invalid credentials');
    });
  });
  ...
  describe('register', () => {
    it('should register user successfully', async () => {
      ...
      // Assert
      expect(mockAssetService.createAsset).toHaveBeenCalledWith(registerPayload.avatar);
      expect(mockHashUtil.generateHash).toHaveBeenCalledWith(registerPayload.password);
    ...
    it('should handle asset creation failure', async () => {
      ...
      mockAssetService.createAsset.mockRejectedValue(new Error('Asset creation failed'));
