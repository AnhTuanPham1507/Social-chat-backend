...
export class MinioService {
    ...
    private async _findOrCreateBucket(bucketName: string, region?: string): Promise<void> {
        const existedBucket = await this._minioClient.bucketExists(bucketName);
        if (!existedBucket) this._minioClient.makeBucket(bucketName, region);
    }
    ...
    async uploadFile(fileBuffer: Buffer, fileName: string, fileSize: number, mimetype: MIME_TYPE): Promise<string> {
