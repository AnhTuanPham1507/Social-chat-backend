...
import { HEADER_VERSIONING, REQ_ID_HEADER } from '@social-chat/common';
                    ...
                    pinoHttp: {
                        ...
                        serializers: {
                            req: (req: Request): any => ({
                                ...
                                headers: {
                                    [REQ_ID_HEADER]:
                                        req.headers[REQ_ID_HEADER] ??
                                        cls?.getId(),
                                    [HEADER_VERSIONING]:
                                        req.headers[HEADER_VERSIONING],
                                },
                            }),
                        ...
                        hooks: {
                            logMethod(args, method, level) {
                                const error =
                                    args[0]?.['err'] ?? args[0]?.['error'];
                                const isError = level >= 50;
                                const context = args[0]?.['context'] ?? '';
                                const shouldWarnOnly =
                                    context.includes('Kafka');
                                    // || error instanceof DomainException; // TODO: Implement DomainException
