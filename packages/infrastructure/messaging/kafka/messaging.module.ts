...
/**
 * Messaging module — provides the Kafka client and producer.
 *
 * Creates a single Kafka instance shared by both producer and consumers.
 * Consumers are registered separately in each service module. Domain events
 * travel in-process via EventEmitterBusAdapter; only integration events go
 * through Kafka.
 *
 * @example
 * ```typescript
...
export class MessagingModule {
  static forRootAsync(options: MessagingModuleAsyncOptions): DynamicModule {
    ...
    return {
      ...
      exports: [KAFKA_CLIENT_TOKEN, KafkaProducerService],
    };
  }
}
