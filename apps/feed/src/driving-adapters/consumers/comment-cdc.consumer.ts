...
@Injectable()
export class CommentCdcConsumer extends CdcBaseConsumer {
    protected readonly topics = [FEED_TOPIC.COMMENT];
    ...
    constructor(
        ...
        super(kafka, CDC_GROUP_ID.COMMENT);
    }

    protected async handleCdcMessage(_table: string, message: DebeziumMessage): Promise<void> {
        await this._feedCdcService.handleCommentChange(message);
    }
}
