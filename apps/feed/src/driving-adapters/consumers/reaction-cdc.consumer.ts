...
@Injectable()
export class ReactionCdcConsumer extends CdcBaseConsumer {
    protected readonly topics = [FEED_TOPIC.REACTION];
    ...
    constructor(
        ...
        super(kafka, CDC_GROUP_ID.REACTION);
    }

    protected async handleCdcMessage(_table: string, message: DebeziumMessage): Promise<void> {
        await this._feedCdcService.handleReactionChange(message);
    }
}
